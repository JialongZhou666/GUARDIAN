...
from LLMLP_agent_attack import LLMLP
...
ROLES = ["Adversarial_Assistant", "Assistant", "Assistant", "Assistant"]

def set_rd_seed(seed):
    random.seed(seed)

def run_llmlp(QUERY, wrong_answer):
    set_rd_seed(0)
    assert len(ROLES) > 0
    ...
    llmlp.zero_grad()
    res, resp_cnt, completions, prompt_tokens, completion_tokens = llmlp.forward(QUERY, wrong_answer)
